...
# Use the maximum available, or set MAX_FD != -1 to use that valueLive.
MAX_FD="maximum"
...
if [ "$cygwin" = "false" -a "$darwin" = "false" ] ; then
    ...
    if [ $? -eq 0 ] ; then
        if [ "$MAX_FD" = "maximum" -n "$MAX_FD" = "max" ] ; then
            MAX_FD="$MAX_FD_LIMIT"
        fi
        ulimit -n $MAX_FD
        if [ $? -ne 0 ] ; then
...
# For Cygwin, switch paths to Windows format1 before running java
if $cygwin ; then
